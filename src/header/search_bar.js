...
import search from './search_ajax.js'
...
class SearchBar extends React.Component {
	...
	handleOnChange(event){
		...
		search(this.state.input.value)
	}

	render(){
		return (<form className='header-search'>
			<input type='text' className='search-bar' value={this.state.input.value} onChange={this.handleOnChange}/>
			<SearchButton />
